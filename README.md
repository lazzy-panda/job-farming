...
- **Facebook-группы/страницы** (`sourceType = facebook`): добавляются по URL `facebook.com/groups/…` на странице «Источники». Требуют cookies живой сессии — скопируйте заголовок `Cookie:` из DevTools в `storage/facebook.cookies.txt` (пример: `storage/facebook.cookies.example.txt`; путь настраивается `FACEBOOK_COOKIE_FILE`). Без cookies источник помечается `facebook_cookies_required` и пропускается. Env: `FACEBOOK_ENABLED` (по умолчанию true), `FACEBOOK_SOURCE_DELAY_MS` (3000). Разметка Facebook нестабильна: коннектор консервативно достаёт пары post_id → текст из embedded-JSON; при смене разметки будет пустой прогон, не мусор.
